...
function buildMeliAnalysisPrompt(user: any, items: any[], orders: any[], visits: any): string {
  ...
  const topItems = items
    ...
    .map(
      (i) =>
        `- ${i.title} (Precio: $${i.price}, Vendidos: ${i.sold_quantity}, Stock: ${i.available_quantity})`
    )
    .join('\n');
...
export function AIAnalysis({ user, items, orders, visits, onClose }: AIAnalysisProps) {
  const { summary, isLoading, error, runSummary } = useAISummary();

  const generateAnalysis = useCallback(() => {
    const jobId = `meli-analysis-${Date.now()}`;
    void runSummary(jobId, buildMeliAnalysisPrompt(user, items, orders, visits));
  }, [user, items, orders, visits, runSummary]);

  useEffect(() => {
    generateAnalysis();
  }, [generateAnalysis]);

  const showLoader = isLoading && summary.length === 0;
          ...
          {showLoader ? (
            <div className="h-full flex flex-col items-center justify-center space-y-6 py-12">
              ...
              <div className="text-center">
                ...
                <p className="text-gray-500">La IA está generando el informe en tiempo real.</p>
              </div>
            </div>
          ) : error ? (
            ...
            <div className="max-w-none">
              <div className="markdown-body bg-white p-8 rounded-2xl shadow-sm border border-gray-100">
                <Streamdown mode="streaming" isAnimating={isLoading} className="prose prose-lg prose-blue max-w-none">
                  {summary}
                </Streamdown>
              </div>
            </div>
